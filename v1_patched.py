...
# @Last Modified time: 2022-04-02 14:08:46
...
def fix_image_url(url: str) -> str:
    """
    old.hypnohub.net still gives hypnohub.net urls for images
    this function makes sure an image links points to old.hypnohub.net
    edit: this seems to not be required anymore for non-deleted posts, cool
    """
    if 'old.hypnohub.net' not in url:
        url = url.replace('hypnohub.net', 'old.hypnohub.net')
    return url
...
def get_deleted_post_image(post: Post) -> str:
    ...
    if SAVE_THUMBNAILS: # the easy way
        return requests.get("https://old.hypnohub.net/hypnohub-hover-zoom.php", params={"post": post.id}).url
    else:
        r = requests.get(f"https://old.hypnohub.net/post/show/{post.id}/")
        r.raise_for_status()
    ...
    def query_fetcher_thread(thread_id: int):
        ...
        with requests.Session() as s:
            while True: # Main scraping loop
                try:
                    r = s.get("https://old.hypnohub.net/post/index.json", timeout=10,
...
def download_queue(queue: Queue[Post], foldername: str) -> Tuple[List[Post], List[Post], List[Post]]:
    ...
    def download_posts_from_queue() -> None:
        ...
        # While there are still posts to download
            ...
            # I could probably put that in a property instead but would be messier
            post.image_url = fix_image_url(post.image_url)
